...
interviews_ppl_room<- interviews %>% mutate(people_per_room = no_membrs / rooms) 
interviews %>% mutate(people_per_room = no_membrs / rooms)

view(interviews_ppl_room)
total_meals <- interviews %>% mutate(total_meals = no_membrs * no_meals) %>% 
...
interviews %>% 
  group_by(village)  %>% 
  summarize (mean = mean(no_membrs),
    max = max(no_membrs),
    min=min(no_membrs),
    n= n()
  )
 
interviews_spread <-interviews %>%mutate(wall_type_logical = TRUE)%>%
